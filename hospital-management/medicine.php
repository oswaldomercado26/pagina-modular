...
if(isset($_POST[submit]))
{
	if(isset($_GET[editid]))
	{
		...
		{
			echo "<script>alert('Registro de medicamentos actualizado con éxito...');</script>";
		}
		else
		{
	...
	else
	{
		...
		if($qsql = mysqli_query($con,$sql))
		{
			echo "<script>alert('Registro de medicina insertado correctamente...');</script>";
		}
		else
		{
			echo mysqli_error($con);
...
<div class="container-fluid">
  ...
  <div class="card">
    ...
    <table class="table table-hover">
      <tbody>
        <tr>
          <td width="34%">Nombre de la Medicina</td>
          <td width="66%"><input placeholder="Ingresar aqui" class="form-control" type="text" name="medicinename" id="medicinename" value="<?php echo $rsedit[medicinename]; ?>" /></td>
        </tr>
        <tr>
          <td width="34%">Precio Medicina</td>
          <td width="66%"><input placeholder="Ingresar aqui" class="form-control" type="text" name="medicinecost" id="medicinecost" value="<?php echo $rsedit[medicinecost]; ?>" /></td>
        </tr>
        <tr>
          <td>Descripcion</td>
          <td><textarea placeholder="Ingresar aqui" class="form-control no-resize" name="description" id="description" cols="45" rows="5"><?php echo $rsedit[description] ; ?></textarea></td>
        </tr>
        <tr>
          <td>Estatus</td>
